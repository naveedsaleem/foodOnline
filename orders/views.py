...
def place_order(request):
    ...
    for i in cart_items:
        ...
        if v_id in k:
            k[v_id] += (fooditem.price * i.quantity)
        else:
            k[v_id] = (fooditem.price * i.quantity)
        ...
        for i in get_tax:
            ...
            tax_amount = round((tax_percentage * k[v_id]) / 100, 2)
            tax_dict.update({tax_type: {str(tax_percentage): str(tax_amount)}})
        # Construct total data
        total_data.update({fooditem.vendor.id: {str(k[v_id]): str(tax_dict)}})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.vendors.add(*vendors_ids)  # add recursively data in many to many fields
            order.save()
            context = {
                'order': order,
...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        for i in cart_items:
            if i.fooditem.vendor.user.email not in to_emails:
                ...
                context = {
                    'order': order,
                    # 'to_email': 'naveed.saleem786@gmail.com',
        ...
        # cart_items.delete()  // Comment for testing purpose
    ...
    return HttpResponse('Paymnts view')
